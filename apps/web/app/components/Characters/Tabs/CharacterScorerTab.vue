...
import {
  ...
  HasAnimatedArt,
} from "~/Core/Utils/CharacterUtils"
...
const { CurrentWeapon, CurrentBuild, CurrentEchoes } = storeToRefs(
...
const IsAnimatedArtVisible = computed(
  () =>
    CurrentCharacter.value
    && !ForceStaticArt.value
    && SettingsStore.GetSetting("Characters").EnableAnimatedArt
    && HasAnimatedArt(CurrentCharacter.value as Character),
)
  ...
  <div v-if="CurrentCharacter">
    <div class="mb-14 xl:mb-4 mt-2 relative mx-auto my-2">
      <div
        ...
        class="relative p-0.25"
      >
        <div
          v-if="ShowScreenShotBackground"
          class="absolute inset-0 blur-sm"
        >
          <div class="absolute -top-5 -left-5 -right-5 -bottom-5 bg-neutral-900/75" />
          <NuxtImg
            :src="GetCharacterBackground(CurrentCharacter.Id)"
